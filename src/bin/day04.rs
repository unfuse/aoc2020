...
fn check_byr(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "byr", |val| val_in_range(val, 1920, 2002)) {
        true
    }
    else {
        println!("  byr has bad val");
        false
    }
}

fn check_iyr(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "iyr", |val| val_in_range(val, 2010, 2020)) {
        true
    }
    else {
        println!("  iyr has bad val");
        false
    }
}

fn check_eyr(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "eyr", |val| val_in_range(val, 2020, 2030)) {
        true
    }
    else {
        println!("  eyr has bad val");
        false
    }
}

fn check_hgt(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "hgt", |val| {
        if val.contains("in") {
            ...
            println!("  hgt has bad val");
            false
        }
}

fn check_hcl(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "hcl", |val| {
        let r = Regex::new("^#[\\da-f]{6}$").unwrap();
        r.is_match(val)
    }) {
            true
        }
...
fn check_ecl(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "ecl", |val| {
        ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(&val)
    }) {
        true
    }
    else {
        println!("  ecl has bad val");
        false
    }
}

fn check_pid(map: &HashMap<String, String>) -> bool {
    if check_map_val(map, "pid", |val| {
        val.len() == 9
    }) {
        true
    }
    else {
        println!("  pid has bad val");
        false
    }
}
